...
// Fallback for saveAs if named export fails
...
const StoryDisplay: React.FC<StoryDisplayProps> = ({ content, isGenerating, settings, onContinue }) => {
  ...
  const handleExportPDF = () => {
    ...
    if (exporter) {
      ...
      console.error("PDF Export failed: html2pdf not found.");
      alert("দুঃখিত, PDF এক্সপোর্ট করতে সমস্যা হয়েছে।");
    }
  };
  ...
  const handleExportDocx = async () => {
    try {
        ...
        sections: [{
          children: [
            ...
            ...content.split('\n').map(line => new Paragraph({
              children: [new TextRun({ text: line, size: fontSize * 2 })], // docx size in half-points
      ...
      if (typeof saveAs === 'function') {
        ...
        throw new Error("saveAs is not a function");
      }
    } catch (e) { 
      ...
      <div className={`max-w-4xl mx-auto shadow-2xl rounded-2xl min-h-[85vh] flex flex-col overflow-hidden border ${theme === 'dark' ? 'border-slate-800' : 'border-slate-100'} ${themeClasses[theme]}`}>
        ...
        <div className="flex-1 overflow-visible relative">
          <div ref={printRef} style={{ padding: `${marginSize}in` }} className="h-full">
            {isPlaceholder ? (
              <div className="h-full min-h-[50vh] flex flex-col items-center justify-center text-center space-y-6 opacity-20 pointer-events-none p-10">
                <svg className="w-24 h-24" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>
                <p className="text-3xl font-black uppercase tracking-tighter">গল্প লিখুন, প্রাণ দিন আপনার কল্পনার</p>
                <p className="text-sm font-bold">(Unleash your imagination with AI Storytelling)</p>
              </div>
            ) : (
              <article className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-1000">
                <header className="border-b-2 pb-10 text-center border-indigo-500/20 mb-12">
                  <h1 className="font-black mb-6 leading-tight bengali-font" style={{ fontSize: `${fontSize * 1.6}px` }}>
                    {settings.title || "গল্পের শিরোনাম"}
                  </h1>
                  <div className="flex justify-center items-center flex-wrap gap-3">
                    <span className="text-[9px] font-black uppercase tracking-[0.1em] bg-indigo-500/10 px-3 py-1 rounded-full text-indigo-600 border border-indigo-500/20">
                      {settings.type}
                    </span>
                ...
                <div 
                  className="story-content leading-[2.2] whitespace-pre-wrap bengali-font select-text text-justify px-4 md:px-8"
        ...
        {content && !isGenerating && (
          <div className={`p-6 border-t flex flex-wrap gap-5 justify-center md:justify-between items-center transition-all ${theme === 'dark' ? 'border-slate-800 bg-slate-900/80' : 'border-slate-100 bg-slate-50/80'}`}>
            ...
            <div className="flex items-center space-x-2 bg-indigo-600/5 p-1.5 rounded-2xl border border-indigo-500/10">
              <button onClick={handleExportPDF} className="px-5 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-white transition-all shadow-sm active:scale-95 text-indigo-700">PDF</button>
              <button onClick={handleExportDocx} className="px-5 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-white transition-all shadow-sm active:scale-95 text-indigo-700">Word</button>
              <button 
                onClick={() => {
